...
export default function TransparencyPage() {
  return (
    <div>
      ...
      <p className="notice">
        Per the challenge rules, <strong>every feature and its purpose is published here</strong>.
        There are no hidden proxies: we do not infer protected traits and do not use demographic,
        behavioral, or landlord-revenue features to rank, score, or decide anything about you.
      </p>
      ...
      <ul className="plain">
        {FEATURES.map((f) => (
          <li key={f.name} className="card">
            <strong>{f.name}</strong> <span className="badge ok">proxy risk: {f.proxyRisk}</span>
            <p>{f.purpose}</p>
      ...
      <p className="notice">
        The following area-level datasets are used only for optional, transparent context. They are{" "}
        <strong>never joined to your profile</strong> and never used to profile, score, or rank an
      ...
      <ul className="plain">
        ...
        <li className="card">
          Hard-delete your session — all stored data is removed, not just flagged.
        </li>
